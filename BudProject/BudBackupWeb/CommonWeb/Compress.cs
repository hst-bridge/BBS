...
namespace budbackup.CommonWeb
{
    public class GZip
    {
        ...
        private static void CreateTempFile(FileInfo[] files, string lpBaseFolder, string lpTempFile, GZipResult result)
        {
            ...
            // 20140415 wangdan add
            string fileCreateDate = null;
            string fileAccessDate = null;
                ...
                try
                {
                    ...
                    foreach (FileInfo fi in files)
                    {
                        ...
                        try
                        {
                            ...
                            fileModDate = fi.LastWriteTime.ToString();
                            fileCreateDate = fi.CreationTime.ToString();
                            fileAccessDate = fi.LastAccessTime.ToString();
                            // 20140415 wangdan add
                            gzf.CreateDate = fi.CreationTime;
                            gzf.ModifiedDate = fi.LastWriteTime;
                            // 20140415 wangdan add
                            gzf.AccessDate = fi.LastAccessTime;
                            gzf.Length = buffer.Length;

                            // 修正
                            fileHeader = fileIndex.ToString() + "," + vpSourceFile + "," + fileModDate + "," + fileCreateDate + "," + fileAccessDate + "," + buffer.Length.ToString() + "\n";
                        ...
                        if (!string.IsNullOrEmpty(line))
                        {
                            ...
                            if (gzf.ParseFileInfo(line) && gzf.Length > 0)
                            {
                                ...
                                if (write)
                                {
                                    ...
                                    // 20140415 wangdan
                                    FileInfo decompressFileInfo = new FileInfo(lpFilePath + addExtension);
                                    if (decompressFileInfo.Exists)
                                    {
                                        decompressFileInfo.CreationTime = gzf.CreateDate;
                                        decompressFileInfo.LastWriteTime = gzf.ModifiedDate;
                                        decompressFileInfo.LastAccessTime = gzf.AccessDate;
                                    }
                                }
                                else
                                {
            ...
            try
            {
                if (!string.IsNullOrEmpty(fileInfo))
                {
                    ...
                    if (info != null && info.Length == 6)
                    {
                        ...
                        this.CreateDate = Convert.ToDateTime(info[3]);
                        this.AccessDate = Convert.ToDateTime(info[4]);
                        this.Length = Convert.ToInt32(info[5]);
                        success = true;
                    }
                }
            }
