...
namespace BudFileListen.Common
{
    public class CopyDirectory
    {
        ...
        private void CopyFolder(DirectoryInfo p_Source, DirectoryInfo p_Target)
        {
            ChildFolderAndFileCopy(p_Source, p_Target);
        }
        ...
        /// <param name="source">コピー元</param>
        /// <param name="target">コピー先</param>
        private void CopyFile(string p_Source, string p_Target)
        {
            try
            {
        ...
        private void ChildFolderAndFileCopy(DirectoryInfo p_Source, DirectoryInfo p_Target)
        {
            // フオルダーの存在性の判断
                ...
                foreach (FileSystemInfo fsi in fsis)
                {
                    if (fsi is FileInfo)
                    {
                        if (fsi.Exists)
                        {
                            ...
                            {
                                logger.Error(p_Target.FullName + e.Message);
                    ...
                    else
                    {
                        try
                        {
                            ...
                            ChildFolderAndFileCopy(sourceSubDir, targetSubDir);
                        }
                        catch (Exception e)
                        {
                            logger.Error(p_Target.FullName + e.Message);
